...
public class UserServiceImpl implements UserService {
    ...
    @Autowired
    private final AuthenticationManager authenticationManager; //used to process an auth request passed to it and returns a fully populated auth object if successful
...
//            List<UserOTPMST> users =userOTPRepository.findOtpValidateByEmail(generateOTPDto.getEmail(), Sort.by("generated_date"));
            UserOTPMST userOtp = userOTPRepository.findOtpValidateByEmail(generateOTPDto.getEmail()).getLast();
    ...
    @Override
    public User updateProfile(UpdateCustomerProfileDto updateCustomerProfileDto, String username) throws UserNotFoundException {

        User user = null;

        user = userRepository.findByUsername(username).orElseThrow(() -> new UserNotFoundException("No Valid User Found"));
        if (user != null) {
            user.setEmail(updateCustomerProfileDto.getEmail());
            user.setContactDetails(updateCustomerProfileDto.getContactDetails());
            user.setCurrency(updateCustomerProfileDto.getCurrency());
            user.setFullname(updateCustomerProfileDto.getFullname());
            user.setMonthly_income(updateCustomerProfileDto.getMonthly_income());
            user.setRec_rem(updateCustomerProfileDto.isRec_rem());
    ...
    @Override
    public User fetchProfile(String username) throws UserNotFoundException {
        //responsible for fetching in user details.
        User user = null;
        user = userRepository.findByUsername(username).orElseThrow(() -> new UserNotFoundException("No Valid User Found"));
        return user;
    }
